...
const CUSTOMER_COLL_ID = process.env.CUSTOMER_COLLECTION_ID;
const TRASH_COLL_ID = process.env.CUSTOMER_TRASH_COLLECTION_ID;
...
router.post("/new", async (req, res) => {
  ...
  try {
    ...
    if (!data.customerId) {
      const existing = await db.listDocuments(DB_ID, CUSTOMER_COLL_ID);
      const count = existing.total + 1;
      const paddedId = String(count).padStart(3, "0");
      data.customerId = `cust-${paddedId}`;
    }
...
router.get("/list", async (req, res) => {
  try {
    ...
    const customers = await db.listDocuments(DB_ID, CUSTOMER_COLL_ID);
    ...
    const enrichedCustomers = customers.documents.map((customer) => ({
      ...customer, // All original fields including createdAt, createdBy, etc.
      employeeName: usernameToName[customer.username] || "Unknown",
    }));
...
router.put("/update/:id", async (req, res) => {
  try {
    ...
    const existingCustomer = await db.getDocument(DB_ID, CUSTOMER_COLL_ID, id);
    ...
    for (const key in updates) {
      ...
      const oldVal = existingCustomer[key];
    ...
    const existingHistory = Array.isArray(existingCustomer.updateHistory)
      ? existingCustomer.updateHistory
      : [];
...
router.delete("/delete/:id", async (req, res) => {
  ...
  try {
    const customer = await db.getDocument(DB_ID, CUSTOMER_COLL_ID, id);

    // Remove Appwrite system fields
    const { $id, $collectionId, $databaseId, ...cleanData } = customer;
    ...
    await db.deleteDocument(DB_ID, CUSTOMER_COLL_ID, id);
